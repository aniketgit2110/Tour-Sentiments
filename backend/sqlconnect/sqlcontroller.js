...
export const getTours = (req, res) => {
  const query =
    "SELECT id, City, Place, Place_desc,Ratings, Distance FROM locations LIMIT 10000;";
...
export const findTours = (req, res) => {
  ...
  FROM locations 
  WHERE City = ? AND Place_desc LIKE ?
...
`;
  ...
  db.query(query, [location, `%${interest}%`, `%${interest}%`], (err, data) => {
    if (err) {
